...
export const toggleUserStatus = async (id,v) =>
  unwrap(await apiClient.patch(`/users/${id}/status?active=${v}`));

export const changeUserRole = async (id,r) =>
  unwrap(await apiClient.patch(`/users/${id}/role?role=${encodeURIComponent(r)}`));

export const adminResetPassword = async (id,p) =>
  unwrap(await apiClient.post(
    `/users/${id}/reset-password?newPassword=${encodeURIComponent(p)}`
  ));
