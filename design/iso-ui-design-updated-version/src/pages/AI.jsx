...
import { getAiConfig, updateAiConfig, getAvailableModels,
         getGlobalPrompt, updateGlobalPrompt, testPrompt,
         getPromptVersions, rollbackPrompt, getAiLogs } from "../api/ai";
...
import { PageHeader, Card, Tag, SmBtn, Btn, RoleBanner,
         LoadingBar, ErrorBanner, Label, Th } from "../components/shared";
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              <Card title="Available Models" badge="Ollama local">
                ...
                {(models||[]).map((m,i) => (
                  <div key={m.name} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 0", borderBottom:i<models.length-1?`1px solid ${T.border}22`:"none" }}>
                    <div style={{ flex:1 }}>
      ...
      {tab==="profiles" && (
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          {profiles?.content?.map(p => <ProfilePromptCard key={p.profileId} profile={p} canEdit={can.edit} />)}
        </div>
      )}
...
function ProfilePromptCard({ profile, canEdit }) {
  ...
  const { data, refetch } = useApi(() => getProfilePrompt(profile.profileId), [profile.profileId]);
  const { mutate: doUpsert } = useMutation((c,n)=>upsertProfilePrompt(profile.profileId,c,n));
  const { mutate: doDelete } = useMutation(()=>deleteProfilePrompt(profile.profileId));
