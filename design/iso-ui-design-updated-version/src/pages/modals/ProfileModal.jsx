...
export default function ProfileModal({ profile, formats, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    formatId:           profile?.formatId           || (formats[0]?.formatId || ""),
    host:               profile?.host               || "",
  ...
  const { mutate: doUpdate, loading: updating } = useMutation((d) => updateProfile(profile.profileId, d));
  ...
  const handleSave = async () => {
    if (!validate()) return;
    try {
      isEdit ? await doUpdate(form) : await doCreate(form);
      onSaved(); onClose();
    } catch (err) {
    ...
    <Modal title={isEdit ? `Edit — ${profile.profileName}` : "New Switch Profile"} onClose={onClose}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        ...
        <Field label="Message Format" required error={errors.formatId}>
          <select value={form.formatId} onChange={e=>set("formatId",+e.target.value)} style={SL}>
            <option value="">Select format…</option>
            {formats.map(f => <option key={f.formatId} value={f.formatId}>{f.formatName}</option>)}
          </select>
        </Field>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8, justifyContent:"center" }}>
          ...
          <ToggleRow label="TPDU / Header" active={form.tpduEnabled} onClick={()=>set("tpduEnabled",!form.tpduEnabled)} />
        </div>
